...
if (scrittaDuration > 0){
	...
	for (var i=0;i <= array_length(listaToccaMuri)-1;i++){
		//storo current element (nome)
		var cur_ele = listaToccaMuri[i].nameChosen
		...
		if (i>0){
			...
			//metto la e
			if (i == array_length(listaToccaMuri)-1){
				spacing = " e "
			}
		}
	...
	var numTxt = "L'HA"
	if (array_length(listaToccaMuri)>1){
		numTxt = "L'HANNO"
	}
